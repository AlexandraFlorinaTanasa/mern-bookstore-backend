...
describe('BookStore API Endpoints', ()=>{
 beforeAll(async()=>{
  ...
  //Daca login-ul reuseste, salveaza token-ul
...
describe('Rute cart', ()=>{
 ...
 it('DELETE /api/cart/:productId ar trebui sa elimine produse din cart', async()=>{
...
describe('Admin Routes- Autentificare necesara', ()=>{
 ...
 if(authToken){
  ...
  it('POST /api/admin/products ar trebui sa creeze un nou produs', async()=>{
   ...
   it('PUT /api/adkin/products/:id ar trebui sa se actualizeze un produs', async()=>{
...
describe('Rute autentificare', ()=>{
 it('POST /api/admin/login ar trebui sa realizeze autentificarea', async()=>{
  ...
  //Accepta atat succes cat si failure - important este ca raspunde 
  expect([200,404]).toContain(response.status);
 ...
 it('POST /api/admin/login ar trebui sa respinga credentialele invalide', async()=>{
  const response=await request(app)
  .post('/api/admin/login')
  .send({
...
.send({
 ...
 describe('Manipulare erori', ()=>{
  it('ar trebui sa manipuleze rute npn-existente', async()=>{
